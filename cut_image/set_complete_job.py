...
def draw_rectangle(event, x, y, flags, param):
    ...
    if(event == cv2.EVENT_LBUTTONDOWN):
        if(len(start_points) == len(end_points)):
            ...
            temp_img = img.copy()
            cv2.circle(temp_img, tuple(start_points[-1]), 11, (0, 255, 0), -1)
            ...
            temp_img = img.copy()
            cv2.circle(temp_img, tuple(start_points[-1]), 11, (0, 255, 0), -1)
            cv2.circle(temp_img, tuple(end_points[-1]), 11, (0, 255, 0), -1)
...
if(len(start_points) == 0 or len(start_points) != len(end_points)):
    ...
    pickle.dump(start_points, open("{}/{}_complete_start_points.pickle".format(recipe_name, ip), "wb"))
    pickle.dump(end_points, open("{}/{}_complete_end_points.pickle".format(recipe_name, ip), "wb"))